...
fn main() {
    let mut horizontal_positions: Vec<u16> = fs::read_to_string("input.txt").expect("Unable to open file").split(",").map(|number| number.parse::<u16>().expect("Unable to convert to u16")).collect();
    ...
    for i in 0..=horizontal_positions.len() - 1 {
        ...
        for position in horizontal_positions.iter(){
            let cost = *position as i16 - i as i16;
            fuel_cost = fuel_cost + cost.abs() as u32;
        }
